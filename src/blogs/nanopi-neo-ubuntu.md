...
> [!IMPORTANT]
> 如果是 **Apple Silicon Mac** 需要 **UTM** 安装一个 **x86_64 Ubuntu server**，因为本文使用的安装 `rootfs` 的工具仅 **x86_64** 系统可用。
...
![](assets/nanopi-neo-ubuntu/fdisk.png)

`sdb1` 是 `boot` 分区，`sdb2` 是 `rootfs` 分区。
官方镜像可能会多一个 `sdb3` 存放 `userdata`，需要删除后两个分区并格式化，如图将末尾分区删除。

![](assets/nanopi-neo-ubuntu/delete.png)

3. 使用如下所示命令创建分区。

![](assets/nanopi-neo-ubuntu/new.png)

4. 其中 `sector` 部分延续 `sdb1` 的 **131071**，第二个直接留空

5. 运行下面的命令将`rootfs`分区格式化
...
1. 在 [Github](https://github.com/u-boot/u-boot/tags) 上下载最新 `u-boot.tar.gz`，这里以 2024.04 为例[^2]

2. 在 [Arm GNU](https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads) 官网按图寻找下载正确的交叉编译链。

![](assets/nanopi-neo-ubuntu/arm-toolchain.png)
...
sudo apt-get install libncurses-dev gawk flex bison \
     ...
     libiberty-dev autoconf dwarves bc
...
make menuconfig
```
![](assets/nanopi-neo-ubuntu/menu-config.png)
...
6. 将 SD 卡插入开发板中，进入 u-boot 命令行拉取内核启动[^4]
...
```
::: warning
很多教程这里会让你换国内源，但是我每次换源后 apt 很多东西装不上，因此请自行选择是否换 apt 源。
换源方法见：[清华 Ubuntu Ports 软件仓库](https://mirrors.tuna.tsinghua.edu.cn/help/ubuntu-ports/)
:::
...
![](assets/nanopi-neo-ubuntu/nmcli.png)
...
![](assets/nanopi-neo-ubuntu/locale.png)
...
按 3 下 <kbd>enter</kbd> 直到出现如图所示内容

![](assets/nanopi-neo-ubuntu/select-locale.png)
...
![](assets/nanopi-neo-ubuntu/language.png)
...
## 配置 Clion 用于 Linux 驱动开发

这篇文章起因是 Clion 无法找到 `file_operations` 结构体，但这样移植之后还是找不到。配置 Clion 远程开发的教程很多，比如[这篇](https://zhuanlan.zhihu.com/p/661990065)，这里不做赘述，仅提出我这个问题的解决办法：
